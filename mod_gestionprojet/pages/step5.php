...
window.collectFormData = function() {
    const formData = {};
    const form = document.getElementById('essaiForm');

    // Regular fields (text inputs, textareas, date)
    form.querySelectorAll('input[type="text"], input[type="date"], textarea').forEach(field => {
        if (field.name && !field.name.startsWith('precaution_')) {
    ...
    const precautions = [];
    for (let i = 1; i <= 6; i++) {
        const precaution = document.getElementById('precaution_' + i);
        if (precaution) {
            precautions.push(precaution.value);
        }
    }
